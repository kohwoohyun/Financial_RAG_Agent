...
def rag_search_tool(query: str) -> str:
    ...
    filter_dict = {}
    if target_company:
        filter_dict["company"] = target_company
    if target_account:
        filter_dict["account"] = target_account
...
tools = [
    ToolDef("재무데이터검색", "삼성전자, SK하이닉스, 현대자동차, LG에너지솔루션, NAVER의 2022~2024년 매출액, 영업이익, 자산총계 등 재무 수치 조회", rag_search_tool),
    ToolDef("웹검색", "최신 뉴스, 시장 동향 등 재무데이터검색에 없는 정보 검색", web_search.run),
]
tool_map = {t.name: t.func for t in tools}

def run_agent(question, max_iterations=5, verbose=True):
...
반드시 먼저 Action으로 도구를 사용한 후, 그 결과(Observation)를 보고 Final Answer를 작성하세요.
...
Thought:"""
    ...
    return "최대 반복 횟수 도달, 답변을 찾지 못했습니다."

question = "삼성전자 최근 반도체 업황 관련 뉴스 알려줘"
