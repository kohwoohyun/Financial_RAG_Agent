...
생성형 AI 기반 PCB 불량 탐지(Vision), 문맥 기반 문장 순서 예측(Language), 전력사용량 예측(Time Series), 언어 기반 이미지 색채화(Multimodal) 프로젝트에 이어, 산업공학 데이터사이언스 분야에서 최근 핵심 화두인 **LLM과 Agentic AI**를 직접 다뤄보기 위해 시작한 프로젝트입니다.
